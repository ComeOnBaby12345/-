...
#define RADIUS 100.0
#define PHOTONUM 5//分类视图个数
#define TAGSTART 1000
#define TIME 0.75//旋转用时
...
- (void)configViews {
    ...
    NSArray *dataArray = @[@"Slice 1", @"Slice 2", @"Slice 3", @"Slice 4",@"Slice 5"];
...
{
    SubClassTableViewController *subClassVc=segue.destinationViewController;
    subClassVc.index=(NSString*)sender;
}
#pragma mark - EFItemViewDelegate
- (void)didTapped:(NSInteger)index {
    if (self.currentTag  == index) {
        //跳转到分类查得界面
        [self performSegueWithIdentifier:@"subClass" sender:[NSString stringWithFormat:@"%lu",index]];
        NSLog(@"%@",[NSString stringWithFormat:@"%lu",index]);
        return;
}
